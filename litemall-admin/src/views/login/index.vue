    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate(valid => {
        ...
        if (valid && !this.loading) {
          ...
          this.$store.dispatch('LoginByUsername', this.loginForm).then((ref) => {
            let role = ref.data.data.menuList
            this.loading = false
            let strBrwaer = ref.data.data.browserEquipSelect
            if(strBrwaer=='isPc'){
              this.$router.push({ path: '/' })
            }else{
              this.$router.replace({ path: '/pad'})
            }
          }).catch(() => {
            this.loading = false
          })
