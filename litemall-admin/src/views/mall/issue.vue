<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" min-width="200px" label="问题类别" prop="questionType">
        <template slot-scope="question">
          <el-tag :type="question.row.questionType ? 'success' : 'error' ">{{question.row.questionType == '0' ? '商品帮助' : '帮助中心'}}</el-tag>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['问题ID','问题类别', '问题内容', '问题回复']
        const filterVal = ['id', 'questionType', 'question', 'answer']
