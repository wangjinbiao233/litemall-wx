<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column v-if = 'false' align="center" min-width="100px" label="级别" prop="level"
      ...
      <el-table-column v-if = 'false' align="center" min-width="100px" label="父类目ID" prop="parentId">
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="dataForm" status-icon label-position="left" label-width="100px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="图标" prop="iconUrl">
          <el-tooltip content="建议图片宽高90*90，或宽高比1:1" placement="top-start" style= "width:90px">
            <el-upload
              class="cate-avatar-uploader"
              v-model="dataForm.iconUrl"
              :action="fileImgUrl"
              :show-file-list="false"
              :on-success="uploadIconUrlSuccess"
              :before-upload="beforeAvatarUpload">
              <template v-if="dataForm.iconUrl">
                <img :src="dataForm.iconUrl" class="cate-avatar" width="90px" height="90px">
              </template>
              <i v-else class="el-icon-plus cate-avatar-uploader-icon" :style = "{width:'90px',height:'90px','line-height':'90px'}"></i>
            </el-upload>
          </el-tooltip>
        </el-form-item>-->
        ...
        <el-form-item label="首页横幅" prop="bannerUrl">
          <el-tooltip content="建议图片宽高1080*360，或宽高比3:1" placement="top-start" style= "width:120px">
            <el-upload
              class="cate-avatar-uploader"
              v-model="dataForm.bannerUrl"
              :action="fileImgUrl"
              :show-file-list="false"
              :on-success="uploadBannerUrlSuccess"
              :before-upload="beforeAvatarUpload">
              <template v-if="dataForm.bannerUrl">
                <img :src="dataForm.bannerUrl" class="cate-avatar" width="360px" height="120px">
              </template>
              <i v-else class="el-icon-plus cate-avatar-uploader-icon" ></i>
            </el-upload>
          </el-tooltip>
        </el-form-item>-->
        ...
        <el-form-item label="类目页横幅" prop="wapBannerUrl">
          <el-tooltip content="建议图片宽高750*246，或宽高比3:1" placement="top-start" style= "width:120px">
            <el-upload
              class="cate-avatar-uploader"
              v-model="dataForm.wapBannerUrl"
              :action="fileImgUrl"
              :show-file-list="false"
              :on-success="uploadWapBannerUrlSuccess"
              :before-upload="beforeAvatarUpload">
              <template v-if="dataForm.wapBannerUrl">
                <img :src="dataForm.wapBannerUrl" class="cate-avatar" width="300px" height="100px">
              </template>
              <i v-else class="el-icon-plus cate-avatar-uploader-icon" ></i>
            </el-upload>
          </el-tooltip>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        keywords: [{ required: true, message: '类目关键字不能为空', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['类目ID', '名称', '关键字', '是否显示', '图标', '首页横幅', '类目页标题', '类目页介绍', '类目页横幅']
        const filterVal = ['id', 'name', 'keywords', 'isShow', 'iconUrl', 'bannerUrl', 'frontName', 'frontDesc', 'wapBannerUrl']
    ...
    uploadIconUrlSuccess(res) {
      this.dataForm.iconUrl = res.data.url
    },
    uploadBannerUrlSuccess(res) {
      this.dataForm.bannerUrl = res.data.url
    },
    uploadWapBannerUrlSuccess(res) {
      this.dataForm.wapBannerUrl = res.data.url
    },
...
<style>
  .cate-avatar-uploader .el-upload {
  ...
  .cate-avatar-uploader .el-upload:hover {
    border-color: #409EFF;
  }
  .cate-avatar-uploader-icon {
    font-size: 28px;
    color: #8c939d;
    width: 120px;
    height: 120px;
