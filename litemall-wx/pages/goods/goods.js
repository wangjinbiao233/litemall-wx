...
Page({
    ...
    util.request(api.GoodsDetail, { id: that.data.id }).then(function (res) {
      if (res.errno === 0) {
        ...
        that.setData({
          ...
          counterPrice: res.data.info.counterPrice
        });
  ...
  onLoad: function (options) {
    ...
    console.log(wx.setStorageSync('storeid',0))
    console.log(wx.setStorageSync('name',''))
    console.log(wx.setStorageSync('img', ''))
  ...
  closeAttrOrCollect: function () {
    ...
    if (this.data.openAttr) {
      ...
      //添加或是取消收藏
      util.request(api.CollectAddOrDelete, { typeId: 0, valueId: this.data.id, userId: wx.getStorageSync('userId') }, "POST")
        .then(function (res) {
          let _res = res;
          if (_res.errno == 0) {
  ...
  addFast: function () {
    ...
    if (this.data.openAttr == false) {
      ...
      //立即购买 this.data.storeid  
      util.request(api.CartFastAdd, { storeId: that.data.storeid, goodsId: this.data.goods.id, number: this.data.number, productId: checkedProduct.id, userId: wx.getStorageSync('userId')},  "POST")
        .then(function (res) {
          if (res.errno == 0) {
  ...
  addToCart: function () {
    ...
    if (this.data.openAttr == false) {
      ...
      util.request(api.CartAdd, { goodsId: this.data.goods.id, number: this.data.number, productId: checkedProduct.id, userId: wx.getStorageSync('userId')}, "POST")
        .then(function (res) {
          let _res = res;
          if (_res.errno == 0) {
            wx.showToast({
  ...
  toSelectStore: function (res) {
 
    var id = Number(res.currentTarget.dataset.id)
  
    wx.navigateTo({
      url: '../shopping/store/store?id='+ id
    })
