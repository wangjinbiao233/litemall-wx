...
Page({
  ...
  goLogin(){
    if (!app.globalData.hasLogin) {
      wx.navigateTo({ url: "/pages/auth/login/login?pId=" + this.data.pId });
    }
  },
  goReservation() {
    wx.navigateTo({ url: "/pages/ucenter/reservation/index" });
  },
  goOrderList(e){
    var sort = e.currentTarget.dataset.sort - 0
    var flag = e.currentTarget.dataset.flag - 0
    wx.navigateTo({ url: "/pages/ucenter/order/index?sort=" + sort + "&flag=" + flag });
  },
  goOrder() {
    if (app.globalData.hasLogin) {
      wx.navigateTo({ url: "/pages/ucenter/order/index" });
    }
    else {
      wx.navigateTo({ url: "/pages/auth/login/login?pId=" + this.data.pId });
    }
      ...
      if (app.globalData.hasLogin) {
        util.request(api.getUserInfo, { userId: wx.getStorageSync('userId') }, 'POST').then(function (res) {
          ...
          if (distributionPartner === true) {
            wx.navigateTo({ url: "/pages/ucenter/distributionIndex/distributionIndex" });
          } else {
            util.request(api.selectApplyByUserId, { userId: wx.getStorageSync('userId') }, 'POST').then(function (res) {  
              if (res.data != null && res.data.auditStatus == 0){
                util.showErrorToast('审核中')
              } else if (res.data != null && res.data.auditStatus == 2){
                wx.navigateTo({ url: "/pages/ucenter/distributionApply/distributionApply" });
              }else{
                wx.showModal({
                  title: '提示',
                  content: '是否成为分销合作伙伴',
  ...
  goRecharge() {    
    if (app.globalData.hasLogin) {
      wx.navigateTo({ url: "/pages/ucenter/rechargeIndex/rechargeIndex" });
    }
    else {
      wx.navigateTo({ url: "/pages/auth/login/login?pId=" + this.data.pId });
    };  
  ...
  goHelpCenter(){
    if (app.globalData.hasLogin) {
      wx.navigateTo({ url: "/pages/ucenter/helpCenter/helpCenter" });
    }
    else {
      wx.navigateTo({ url: "/pages/auth/login/login?pId=" + this.data.pId  });
    };
