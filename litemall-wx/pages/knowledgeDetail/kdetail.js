...
Page({
  ...
  getKnowledge:function(){
    let that = this;
    util.request(api.KnowledgeDetail,{id:this.data.id}).then((res)=>{
      that.setData({
  ...
  getComment: function () {
      let that = this;
      util.request(api.KnowledgeComment, { userId: wx.getStorageSync('userId'), valueId: this.data.id, typeId: 2 }).then((res) => {
          console.log(res);
          that.setData({
