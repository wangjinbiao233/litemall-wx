...
public class BaseCommandOption {
    ...
    public static final String FORMAT_COPY = "copy";
...
//                shotSecond = 1;
//            }
            //random select screenshot
            if(shotSecond==0) {
            	Random rand = new Random();
            	shotSecond=rand.nextInt((int)vi.getDuration()-1)+1;
            }
//            System.out.println("getDuration"+vi.getDuration());
//            System.out.println("shotSecond"+shotSecond);
            commands.add(SS);
    ...
    /**
     * 针对部分mp4视频重新输出，解决卡顿问题
     * ffmpeg -i S202Teddysdayout.mp4 -acodec copy -vcodec copy S202Teddysdayout_copy.mp4
     * @param input
     * @param output
     * @param vi
     * @return
     */
    public static List<String> toMP4GerneralCopyCmdArrays(String input, String output, VideoInfo vi) {
        if (vi != null && vi.getSize() > 0) {
            List<String> commands = Lists.newArrayList(toInputCommonsCmdArrays(input));
            commands.addAll(getRoateCmdArrays(vi));
            commands.addAll(Lists.newArrayList(
